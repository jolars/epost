...
- **Step 5 (maildir flags)** — in progress. `mail/flags.rs` owns spec-correct `:2,FLAGS` parsing and the rename primitive (shared with the future folder-move feature). Auto-`S` fires once per opened body inside `ensure_body_for_selection`; manual `m` (Normal, List focused) toggles Seen. Index + in-memory `ThreadedRow` mirrored on every flip. `store/watch.rs` exposes a `SelfWrites` registry stub the Step 7 notify watcher will consult.
