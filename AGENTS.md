...
- **Step 6 (compose path)** — done. `mail/compose.rs` builds MIME via `mail-builder`, pipes to `msmtp -t`, drops a `Sent/cur` copy. `ui/compose.rs` + `ui/embed.rs` host `$EDITOR` under a pty (vt100 capability callbacks, env passthrough, synchronized output). Cmdline: `:compose` / `:reply` / `:reply-all` / `:forward` / `:send` / `:close`. Open gaps that are *not* blockers: attachments (multipart/mixed), Drafts/cur persistence across restart, address completion.
...
- Step 7 — notify watcher. See `DESIGN.md`.
